<x-layout>
  <div class="flex gap-2 items-center">
    ...
    <h1 class="dark:text-white text-2xl font-bold">Edit Chess Book</h1>
  </div>
  <section class="mt-5">
    <form method="POST" action="{{ route('bookUpdate', ['id' => $book->id]) }}" enctype="multipart/form-data">
      ...
      <section class="form-grid-3 mb-4">
        ...
        <div>
          ...
          <p>Current File: <a href="{{ $book->book_file }}" target="_blank" class="text-blue-500 underline underline-offset-1">{{ $book->book_file }}</a></p>
        </div>
      </section>
      <div id="imageDiv" class="mb-4">
        <label class="form-label">Preview Image: </label>
        <div id="" class="w-64">
          <img id="preview-img" src="{{ $book->image }}" alt="">
        </div>
      </div>
